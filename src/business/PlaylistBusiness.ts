...
export class PlaylistBusiness {
  ...
  public likeOrDislikePlaylist = async (
    ...
    const { playlistId, token, like } = input;
    ...
    if (payload && payload.id === playlistDBWithCreatorName.creator_id) {
      throw new ConflictError(
        "Você não pode dar like ou deslike na playlist que você criou!"
      );
    }
    ...
    if (likeDislikeExists === PLAYLIST_LIKE.LIKED) {
      if (like) {
    ...
    } else if (likeDislikeExists === PLAYLIST_LIKE.DISLIKED) {
      if (like === false) {
        await this.playlistDatabase.removeLikeDislike(likeDislikeDB);
